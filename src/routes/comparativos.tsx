  ...
  head: () => ({
    meta: [
      { title: "Comparativos de tecnologia | Tech Escolha Certa" },
      {
        name: "description",
        content:
          "Comparativos lado a lado de celulares, notebooks e smartwatches para decidir com clareza.",
      },
      { property: "og:title", content: "Comparativos — Tech Escolha Certa" },
      {
        property: "og:description",
        content: "Tabelas comparativas, vereditos e recomendações por perfil.",
      },
    ],
  }),
    ...
    <>
      ...
      <section className="container-tec py-14 md:py-20">
        <SectionHeader
          eyebrow="Em destaque"
          title="iPhone 15 vs Galaxy S24"
          description="Lado a lado, item por item, sem suspense."
        />
        ...
        <div className="mt-8 rounded-2xl border border-border bg-surface p-6">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <a
              href="/comparativo/iphone-15-vs-galaxy-s24"
              className="inline-flex items-center gap-2 rounded-md bg-cta px-5 py-2.5 text-sm font-semibold text-cta-foreground hover:brightness-95"
            >
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {FEATURED_COMPARISONS.map((c) => (
              <article key={c.title} className="rounded-2xl border border-border bg-card p-6 shadow-soft">
                ...
                <a href={c.href} className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-cta">
            ...
            {COMPARISONS.map((c) => (
              <article key={c.title} className="rounded-2xl border border-border bg-card p-6 shadow-soft">
                ...
                <a
                  href="/comparativo/iphone-15-vs-galaxy-s24"
                  className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-cta"
                >
                  Ver comparativo <ArrowRight className="h-4 w-4" />
                </a>
              </article>
